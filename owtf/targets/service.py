...
from owtf.sessions.service import session_required, add_target_to_session
from owtf.targets.models import Target
...
def set_target(target_id):
    ...
    try:
        target_id = target_id
        target_config = get_target_config_by_id(target_id)
        path_config = get_path_configs(target_config)
    except InvalidTargetReference:
        raise InvalidTargetReference("Target doesn't exist: %s" % str(target_id))


def get_path_configs(target_config):
    ...
    path_config['host_output'] = os.path.join(config.get_val('OUTPUT_PATH'),
    ...
    path_config['url_output'] = os.path.join(config.get_target_dir(target_config['target_url']))
...
def get_path(output_type):
    return path_config.get(output_type, None)

def set_path(output_type, path):
    # Mainly used for setting output paths for individual plugins, which
    # need not be saved: plugin_output_dir.
    path_config[output_type] = path
...
def add_target(target_url, session_id=None):
    ...
    :rtype: None
    """
    if target_url not in get_target_urls():
        # A try-except can be used here, but then ip-resolution takes time
        # even if target is present
        target_config = config.derive_config_from_url(target_url)
        ...
        config_obj.sessions.append(db.session.query(Session).get(session_id))
        db.session.commit()
        target_id = config_obj.id
        create_missing_dirs_target(target_url)
        set_target(target_id)
    else:
        session_obj = db.session.query(Session).get(session_id)
        target_obj = db.session.query(Target).filter_by(target_url=target_url).one()
...
def update_target(data_dict, target_url=None, id=None):
    ...
    target_obj = None
    if id:
        target_obj = db.session.query(Target).get(id)
    if target_url:
        target_obj = db.session.query(Target).filter_by(target_url=target_url).one()
    if not target_obj:
...
def delete_target(target_url=None, id=None):
    ...
    db.session.commit()
    config.cleanup_target_dirs(target_url)
...
def create_missing_dirs_target(target_url):
    ...
    :rtype: None
    """
    config.create_output_dir_target(target_url)
...
def _generate_query(filter_data, session_id, for_stats=False):
    """Generate query
    ...
    :rtype:
    """
    query = db.session.query(Target).filter(Target.sessions.any(id=session_id))
    if filter_data.get("search") is not None:
        ...
        if filter_data.get("scope", None):
            ...
            query = query.filter_by(scope=config.ConvertStrToBool(filter_data.get("scope")))
...
def search_target_configs(filter_data=None, session_id=None):
    ...
    :rtype: `dict`
    """
    total = db.session.query(Target).filter(Target.sessions.any(id=session_id)).count()
    filtered_target_objs = _generate_query(filter_data, session_id).all()
    filtered_number = _generate_query(filter_data, session_id, for_stats=True).count()
    results = {
        "records_total": total,
...
def get_target_config_dicts(filter_data=None, session_id=None):
    ...
    if filter_data is None:
        filter_data = {}
    target_obj_list = _generate_query(filter_data, session_id).all()
...
def is_url_in_scope(url):
    ...
    for host_name in get_all('host_name'):
...
def get_just_finished_targets(self):
    ...
    completed_targets = db.session.query(PluginOutput.target_id).filter(PluginOutput.end_time>=time15min).distinct(PluginOutput.target_id)
    running_targets = db.session.query(Work.target_id).distinct(Work.target_id)
    ...
    for target in recently_finished_targets:
        values = {}
        target_obj = db.session.query(Target).filter_by(id=target)
...
def get_targets_by_severity_count(session_id=None):
    ...
    total = db.session.query(Target).filter(Target.sessions.any(id=session_id)).count()
    target_objs = db.session.query(Target).filter(Target.sessions.any(id=session_id)).all()
