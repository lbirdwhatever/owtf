...
from owtf.transactions.models import Transaction, GrepOutput, Url
...
def num_transactions(scope=True, target_id=None):
    ...
    :rtype: `int`
    """
    count = db.session.query(Transaction).filter_by(scope=scope, target_id=target_id).count()
    return count


@target_required
def is_already_added(criteria, target_id=None):
    ...
    :rtype: `bool`
    """
    return len(get_all(criteria, target_id=target_id)) > 0


def gen_query(criteria, target_id, for_stats=False):
    ...
    :rtype:
    """
    query = db.session.query(Transaction).filter_by(target_id=target_id)
    ...
    if criteria.get('scope', None):
        ...
        query = query.filter_by(scope=config.str2bool(criteria['scope']))
    if criteria.get('binary_response', None):
        ...
        query = query.filter_by(binary_response=config.str2bool(criteria['binary_response']))
...
def get_first(criteria, target_id=None):
    ...
    :rtype:
    """
    query = gen_query(criteria, target_id)
    return get_transaction(query.first())


@target_required
def get_all(criteria, target_id=None):
...
def get_transactions(transactions):
    ...
    :rtype: `list`
    """
    return [get_transaction(transaction) for transaction in transactions if
            get_transaction(transaction) is not None]
            ...
            if grep_outputs:  # If valid grep results exist
                ...
                for regex_name, regex_results in grep_outputs.items():
                    ...
                    for match in regex_results:
                        ...
                        existing_grep_output = db.session.query(GrepOutput).filter_by(
                        ...
                        if existing_grep_output:
                            ...
                            db.session.add(GrepOutput(target_id=target_id,
                                                                    ...
                                                                    output=match))
    db.session.commit()
    url_manager.import_processed_url(urls_list, target_id=target_id)
...
def delete_transaction(transaction_id, target_id=None):
    ...
    :rtype: None
    """
    db.session.query(Transaction).filter_by(target_id=target_id, id=transaction_id).delete()
    db.session.commit()
...
def get_by_id(id):
    ...
    try:
        id = int(id)
        model_obj = db.session.query(Transaction).get(id)
    except ValueError:
        pass
    finally:
...
def get_top_by_speed(order="Desc", num=10, target_id=None):
    ...
    """
    if order == "Desc":
        results = db.session.query(Transaction).filter_by(target_id=target_id).order_by(
            desc(Transaction.time)).limit(num)
    else:
        results = db.session.query(Transaction).filter_by(target_id=target_id).order_by(
            asc(Transaction.time)).limit(num)
...
def compile_regex():
    ...
    :rtype: None
    """
    for key in list(config.get_framework_config_dict().keys()):
        key = key[3:-3]  # Remove "@@@"
        if key.startswith('HEADERS'):
            header_list = config.get_header_list(key)
            regexs['HEADERS'][key] = compile_header_regex(header_list)
        elif key.startswith('RESPONSE'):
            _, _, python_regexp = config.get_val(key).split('_____')
            regexs['BODY'][key] = compile_response_regex(python_regexp)
...
def grep_transaction(owtf_transaction):
    ...
    """
    grep_output = {}
    for regex_name, regex in list(regexs['HEADERS'].items()):
        grep_output.update(grep_response_headers(regex_name, regex, owtf_transaction))
    for regex_name, regex in list(regexs['BODY'].items()):
...
def search_by_regex_name(regex_name, stats=False, target_id=None):
    ...
    grep_outputs = db.session.query(GrepOutput.output).filter_by(
    ...
    for grep_output in grep_outputs:
        transaction_ids.append(db.session.query(Transaction.id).join(
            Transaction.grep_outputs).filter(
                GrepOutput.output == grep_output,
                GrepOutput.target_id == target_id).limit(1).all()[0][0])
    # Calculate stats if needed
    if stats:
        # Calculate the total number of matches
        num_matched_transactions = db.session.query(Transaction).join(
            Transaction.grep_outputs).filter(
        ...
        num_transactions_in_scope = db.session.query(Transaction).filter_by(
...
@target_required
def search_all(criteria, target_id=None, include_raw_data=True):
    ...
    :rtype: `dict`
    """
    total = db.session.query(Transaction).filter_by(target_id=target_id).count()
    filtered_transaction_objs = gen_query(criteria, target_id).all()
    filtered_number = gen_query(criteria, target_id, for_stats=True).count()
    results = {
        "records_total": total,
...
@target_required
def get_all_as_dicts(criteria, target_id=None, include_raw_data=False):
    """Assemble ALL transactions that match the criteria from DB.
...
def get_by_id_as_dict(trans_id, target_id=None):
    ...
    :rtype: `dict`
    """
    transaction_obj = db.session.query(Transaction).filter_by(target_id=target_id, id=trans_id).first()
