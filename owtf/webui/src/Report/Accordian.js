...
import {TARGET_API_URI, WORKLIST_API_URI} from '../Constants.js';
...
class Accordian extends Component {
    ...
    patchUserRank(group, type, code, user_rank) {
        ...
        $.get(TARGET_API_URI + target_id, function (result) {
            ...
            if (localMax == 0) 
                rankHtml = "<i><small><label class='alert alert-passing' style='margin-bottom: 0px'>Passing<" +
                        "/label></small></i>";
            else if (localMax == 1) 
                rankHtml = "<i><small><label class='alert alert-success' style='margin-bottom: 0px'>Info</la" +
                        "bel></small></i>";
            else if (localMax == 2) 
                rankHtml = "<i><small><label class='alert alert-info' style='margin-bottom: 0px'>Low</label>" +
                        "</small></i>";
            else if (localMax == 3) 
                rankHtml = "<i><small><label class='alert alert-warning' style='margin-bottom: 0px'>Medium</" +
                        "label></small></i>";
            else if (localMax == 4) 
                rankHtml = "<i><small><label class='alert alert-danger' style='margin-bottom: 0px'>High</lab" +
                        "el></small></i>";
            else if (localMax == 5) 
                rankHtml = "<i><small><label class='alert alert-critical' style='margin-bottom: 0px'>Critica" +
                        "l</label></small></i>";
    ...
    postToWorkList(selectedPluginData, force_overwrite) {
        selectedPluginData["id"] = document
            .getElementById("report")
            .getAttribute("data-code");
                                    ...
                                    {pluginData
                                        .map(function (obj) {
                                            if ((selectedType.length === 0 || selectedType.indexOf(obj['plugin_type']) !== -1) && (selectedGroup.length === 0 || selectedGroup.indexOf(obj['plugin_group']) !== -1) && (selectedRank.length === 0 || selectedRank.indexOf(obj['user_rank']) !== -1) && (selectedOwtfRank.length === 0 || selectedOwtfRank.indexOf(obj['owtf_rank']) !== -1) && (selectedStatus.length === 0 || selectedStatus.indexOf(obj['status']) !== -1)) {
                                                return (
                                                    <button
                                                        onClick={handlePluginBtnOnAccordian.bind(this, obj['plugin_type'])}
                                                        key={code + obj['plugin_type']
                                                        .split('_')
                                                        .join(' ')}
                                                        className={(() => {
                                                        if (testCaseMax == 0) 
                                                        ...
                                                        type="button">{obj['plugin_type']
                                                            .split('_')
                                                            .join(' ')
                                                            .charAt(0)
                                                            .toUpperCase() + obj['plugin_type']
                                                            .split('_')
                                                            .join(' ')
                                                            .slice(1)}</button>
                                                );
