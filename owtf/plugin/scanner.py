...
def ping_sweep(target, scantype):
    ...
    if scantype == "arp":
        ...
        shell.shell_exec("nmap -n -v -sP -PR %s -oA %s" % (target, PING_SWEEP_FILE))

    shell.shell_exec('grep Up %s.gnmap | cut -f2 -d\" \" > %s.ips' % (PING_SWEEP_FILE, PING_SWEEP_FILE))
...
def dns_sweep(file_with_ips, file_prefix):
    ...
    shell.shell_exec("nmap -PN -n -sS -p 53 -iL %s -oA %s" % (file_with_ips, file_prefix))

    # Step 2 - Extract IPs
    dns_servers = "%s.dns_server.ips" % file_prefix
    shell.shell_exec('grep \"53/open/tcp\" %s.gnmap | cut -f 2 -d \" \" > %s' % (file_prefix, dns_servers))
    ...
    for line in file:
        ...
        shell.shell_exec("host -l %s %s | grep %s > %s" % (domain, dns_server, domain, raw_axfr))
...
def scan_and_grab_banners(file_with_ips, file_prefix, scan_type, nmap_options):
    ...
    if scan_type == "udp":
            ...
            shell.shell_exec("nmap -PN -n -v --min-parallelism=10 -iL %s -sU -sV -O -oA %s.udp %s" % (
                file_with_ips, file_prefix, nmap_options))
            shell.shell_exec("amap -1 -i %s.udp.gnmap -Abq -m -o %s.udp.amap" % (file_prefix, file_prefix))
...
def probe_service_for_hosts(nmap_file, target):
    ...
    for service in services:
        if plugin_list.count(service) > 0:
            continue
        tasks_for_service = len(target_service(nmap_file, service).split("##")) - 1
...
def probe_network(target, protocol, port):
    ...
    :rtype: None
    """
    scan_and_grab_banners("%s.ips" % PING_SWEEP_FILE, FAST_SCAN_FILE, protocol, "-p %s" % port)
