...
from owtf.plugin.models import Plugin
...
def get_test_groups_config(file_path):
    ...
    for line in config_file:
        ...
        try:
            ...
            error_handler.abort_framework("Problem in Test Groups file: '%s' -> Cannot parse line: %s" %
                                                (file_path, line))
        if len(descrip) < 2:
            descrip = hint
        if len(hint) < 2:
            hint = ""
        test_groups.append({'code': code, 'priority': priority,
                            'descrip': descrip, 'hint': hint, 'url': url})
    return test_groups


def load_test_groups(test_groups_file, plugin_group):
    ...
    :rtype: None
    """
    test_groups = get_test_groups_config(test_groups_file)
...
def load_plugins(self):
    ...
    # 'PLUGIN_DIR'.
    plugins = []
    for root, _, files in os.walk(get_val('PLUGINS_DIR')):
        plugins.extend([os.path.join(root, filename)
    ...
    for plugin_path in plugins:
        ...
        plugin = plugin_path.replace(get_val('PLUGINS_DIR'), '')
        ...
        if db.session.query(TestGroup).get(code) is None:
            continue
...
def get_test_group(code):
    ...
    :rtype: `dict`
    """
    group = db.session.query(TestGroup).get(code)
...
def get_all_test_groups(self):
    ...
    :rtype:
    """
    test_groups = db.session.query(TestGroup).order_by(
        TestGroup.priority.desc()).all()
...
def get_all_plugin_types(self):
    ...
    :rtype: `list`
    """
    plugin_types = db.session.query(
        Plugin.type).distinct().all()
    # Necessary because of sqlalchemy
...
def get_types_for_plugin_group(plugin_group):
    ...
    :rtype: `list`
    """
    plugin_types = db.session.query(Plugin.type).filter_by(
        group=plugin_group).distinct().all()
    plugin_types = [i[0] for i in plugin_types]
    return plugin_types
...
def gen_query(criteria):
    ...
    :rtype:
    """
    query = db.session.query(Plugin).join(TestGroup)
    ...
    if criteria.get("group", None):
        ...
        if isinstance(criteria["group"], list):
            query = query.filter(
                Plugin.group.in_(criteria["group"]))
...
def get_plugins_by_type(plugin_type):
    ...
    :rtype: `list`
    """
    return get_all({"type": plugin_type})
...
def get_plugins_by_group(plugin_group):
    ...
    :rtype: `list`
    """
    return get_all({"group": plugin_group})
...
def get_plugins_by_group_type(plugin_group, plugin_type):
    ...
    :rtype: `list`
    """
    return get_all({"type": plugin_type, "group": plugin_group})
...
def get_groups_for_plugins(plugins):
    ...
    :rtype: `list`
    """
    groups = db.session.query(Plugin.group).filter(
        or_(Plugin.code.in_(plugins),
