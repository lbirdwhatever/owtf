...
from owtf.session.service import session_required
from owtf.exceptions import InvalidParameterType
from owtf.plugins import models
...
def plugin_count_output(self):
    ...
    left_count += get_component("worker_manager").get_busy_workers()
...
@target_required
def get_output_dict(obj, target_id=None, inc_output=False):
    ...
    if obj:
        ...
        if inc_output:
            ...
            pdict.pop("output")
        pdict["start_time"] = obj.start_time.strftime(db_config.get("DATE_TIME_FORMAT"))
        pdict["end_time"] = obj.end_time.strftime(db_config.get("DATE_TIME_FORMAT"))
        pdict["run_time"] = timer.get_time_as_str(obj.run_time)
        return pdict

@target_required
def get_output_dicts(obj_list, target_id=None, inc_output=False):
    ...
    dict_list = []
    for obj in obj_list:
        dict_list.append(get_output_dict(obj, target_id=target_id, inc_output=inc_output))
    return dict_list

def gen_query(filter_data, target_id, for_delete=False):
    """Generate query
...
def get_all(filter_data=None, target_id=None, inc_output=False):
    ...
    if not filter_data:
        filter_data = {}
    target.set_target(target_id)
    query = gen_query(filter_data, target_id)
    results = query.all()
    return get_output_dicts(results, target_id=target_id, inc_output=inc_output)

@target_required
def get_unique(target_id=None):
...
@target_required
def delete_all(filter_data, target_id=None):
    ...
    query = gen_query(filter_data, target_id, for_delete=True)
    # Delete the folders created for these plugins
    for plugin in query.all():
        # First check if path exists in db
        if plugin.output_path:
            output_path = os.path.join(config.get_output_dir_target(), plugin.output_path)
...
@target_required
def update(plugin_group, plugin_type, plugin_code, patch_data, target_id=None):
    """Update output in DB
    ...
    query = gen_query(plugin_dict, target_id)
    obj = query.first()
    if obj:
        try:
...
def save_plugin_output(plugin, output, target_id=None):
    ...
    db.session.merge(models.PluginOutput(
        ...
        output_path=(plugin["output_path"] if os.path.exists(
            plugin_handler.get_plugin_output_dir(plugin)) else None),
