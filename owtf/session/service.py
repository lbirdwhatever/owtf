...
def _ensure_default_session(self):
    ...
    :rtype: None
    """
    if db.session.query(Session).count() == 0:
...
def get_session_id():
    ...
    :rtype: `int`
    """
    session_id = db.session.query(Session.id).filter_by(active=True).first()
    return session_id
...
def add_session(session_name):
    ...
    :rtype: None
    """
    existing_obj = db.session.query(Session).filter_by(name=session_name).first()
...
def add_target_to_session(target_id, session_id=None):
    ...
    target_obj = db.session.query(Target).get(target_id)
    if session_obj is None:
...
def generate_query(filter_data=None):
...
def get_all(filter_data):
    ...
    :rtype: `dict`
    """
    session_objs = generate_query(filter_data).all()
